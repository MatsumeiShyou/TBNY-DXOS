...
function Fix-File {
    param([string]$relative_path, [hashtable]$replacements)
    $path = Join-Path (Get-Location) $relative_path
    if (Test-Path $path) {
        Write-Host "Processing $relative_path ..."
        $lines = Get-Content -Path $path
        foreach ($line_num in $replacements.Keys) {
            $index = [int]$line_num - 1
            if ($index -lt $lines.Count) {
                $lines[$index] = $replacements[$line_num]
            }
        }
...
# 1. NumericKeypad.tsx
Fix-File "src/features/repaper-route/driver/sandbox/components/NumericKeypad.tsx" @{
...
# 2. EndShiftPage.tsx
Fix-File "src/features/repaper-route/driver/sandbox/pages/EndShiftPage.tsx" @{
...
Fix-File "src/features/repaper-route/driver/sandbox/pages/StopDetailPage.tsx" @{
...
Fix-File "src/features/repaper-route/driver/sandbox/pages/InspectionPage.tsx" @{
...
# 5. DriverApp.tsx
Fix-File "src/features/repaper-route/driver/sandbox/DriverApp.tsx" @{
...
# 6. FuelPage.tsx
Fix-File "src/features/repaper-route/driver/sandbox/pages/FuelPage.tsx" @{
