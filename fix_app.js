...
c = c.replace(
  /const handleDoubleClickJob = \(job, cellInfo\) => \{\s*if \(job\.isUnregistered\) \{[\s\S]*?return;\s*\}/,
  const handleDoubleClickJob = (job, cellInfo) => {
    if (job.isDeleted) {
      const titleWithoutWarning = job.title ? job.title.replace(/^??削除済\s*/, '') : '';
    ...
    if (job.isSuspended) {
      const titleWithoutWarning = job.title ? job.title.replace(/^??停止中\s*/, '') : '';
      alert(\\ は現在マスタで「一時停止」に設定されています。顧客管理画面から解除してください。\);
      return;
    }
);
