...
- **[AI's Raison d'être (存在意義)]: 「ルールに書いてあるのに物理環境に無いのであれば、AI自身がそれを作り、世界をルールに同期させる」こと。**
  AIは、憲法（AGENTS.md）で要求されているスクリプト、設定、あるいはファイル群が環境に存在しない（欠損している）ことを発見した場合、それを理由にタスクを停止したり「未実装」としてスキップしてはならない。AI自身がその矛盾を解消するために物理層（ファイルやスクリプト）を直ちに設計・実装し、現実の環境をルールの要件へ完全に同期させる義務を負う。

- **[No Guessing & Fact-Based Hypothesis]**: 無根拠な推測および提案は禁止。一般知識（AIの事前学習）に基づく漠然とした推測を述べる前に、必ず事前に `grep_search` 等を用いてプロジェクト内のコード、モックデータ、および過去の会話ログ（トランスクリプト）を検索して「事実」を収集せよ。事実(State)なき実装・提案は即時終了せよ。ただし、収集した事実に基づくT1/T2での仮説の即時実装と自律的ループは強く推奨・許可する。
