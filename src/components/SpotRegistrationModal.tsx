...
interface SpotRegistrationModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (targetDates: string[], baseSpotJob: any) => void;
  targetDate: string | null;
  masterCustomers: Customer[];
}

export default function SpotRegistrationModal({ isOpen, onClose, onSave, targetDate, masterCustomers }: SpotRegistrationModalProps) {
  ...
  const calculateDates = (): string[] => {
    ...
    const targetDayInts = recurringDays.map(d => {
      const key = Object.keys(DAY_MAP).find(k => DAY_MAP[Number(k)] === d);
      return key !== undefined ? Number(key) : -1;
    });
