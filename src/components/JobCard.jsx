...
export default function JobCard({
  ...
  return (
    <div 
      className={`px-1 py-0.5 group/card transition-transform duration-75 absolute top-0 left-0 w-full rounded shadow-sm border p-1 text-xs font-bold leading-tight flex flex-col justify-center ${job.isUnregistered ? 'bg-orange-50 text-orange-900 border-2 border-dashed border-orange-500' : job.isVehicleError ? 'bg-red-100 text-red-900 border-red-500' : `${(jobColorMap[job.id] || COLOR_PALETTE[0]).bg} ${(jobColorMap[job.id] || COLOR_PALETTE[0]).text} ${(jobColorMap[job.id] || COLOR_PALETTE[0]).border}`} ${draggingJobId === job.id ? 'opacity-40 shadow-none ring-0' : 'hover:brightness-95'} ${selectedJobId === job.id ? 'ring-2 ring-blue-600 z-40' : 'z-20'}`}
      ...
      onClick={(e) => { e.stopPropagation(); setSelectedJobId(job.id); setSelectedCell(null); }}
      onDoubleClick={(e) => { 
        e.stopPropagation(); 
        if (onDoubleClickJob) {
    ...
    >
      ...
      <div className="absolute left-0 top-0 bottom-0 w-6 flex items-center justify-center cursor-grab active:cursor-grabbing z-20 hover:bg-black/5 rounded-l" onMouseDown={(e) => { e.stopPropagation(); if (e.button === 2) e.preventDefault(); setDraggingJobId(job.id); setSelectedJobId(job.id); setSelectedCell(null); setDragButton(e.button); setDragOffset({ x: e.clientX, y: e.clientY }); setDragCurrent({ x: 0, y: 0 }); setDragMousePos({ x: e.clientX, y: e.clientY }); }}>
