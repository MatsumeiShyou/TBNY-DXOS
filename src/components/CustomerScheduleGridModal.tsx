...
export default function CustomerScheduleGridModal({ customers, onSave, onClose }: CustomerScheduleGridModalProps) {
  ...
  // 曜日詳細ダイアログ用ステート
  const [activeDayDialog, setActiveDayDialog] = useState<{ customerId: string; dayKey: string } | null>(null);
  const [dayDialogRules, setDayDialogRules] = useState<string[]>([]);
  ...
  const openDayRuleDialog = (customerId: string, dayKey: string) => {
  ...
  const handleToggleSpot = (customerId: string) => {
    setLocalCustomers(prev => prev.map(c => {
  ...
  const handleCustomScheduleChange = (customerId: string, value: string) => {
    setLocalCustomers(prev => prev.map(c => {
      if (c.id !== customerId) return c;
