...
interface CalendarViewProps {
  monthlyExceptions: Record<string, MonthlyException>;
  masterCustomers: Customer[];
  onChangeDate?: (date: Date) => void;
  setViewMode?: (mode: 'dispatch' | 'calendar') => void;
  currentDate?: Date;
  addSpotJob?: (targetDates: string[], spotJob: Job) => void;
  deleteJobFromCalendar?: (dateStr: string, jobId: string, scope?: 'all' | 'future' | 'this', seriesId?: string) => void;
  moveSpotJob?: (sourceDate: string, targetDate: string, jobId: string) => void;
}
...
export default function CalendarView({ 
  ...
  const [spotModalState, setSpotModalState] = useState<{ isOpen: boolean; dateStr: string | null }>({ isOpen: false, dateStr: null });
  const [deleteModalState, setDeleteModalState] = useState<{ isOpen: boolean; dateStr: string | null; jobId: string | null; seriesId: string | null }>({ isOpen: false, dateStr: null, jobId: null, seriesId: null });

  const handleOpenSpotModal = (dateStr: string) => {
  ...
  const handleSaveSpot = (targetDates: string[], spotJob: Job) => {
    if (addSpotJob) addSpotJob(targetDates, spotJob);
  };

  const handleDeleteJob = (dateString: string, job: Job) => {
    if (job.seriesId) {
  ...
  const handleConfirmDelete = (scope: 'all' | 'future' | 'this') => {
    if (deleteJobFromCalendar && deleteModalState.jobId && deleteModalState.dateStr) {
      deleteJobFromCalendar(deleteModalState.dateStr, deleteModalState.jobId, scope, deleteModalState.seriesId || undefined);
    }
    setDeleteModalState({ isOpen: false, dateStr: null, jobId: null, seriesId: null });
  };

  const handleMoveJob = (sourceDate: string, targetDate: string, jobId: string) => {
    if (moveSpotJob) moveSpotJob(sourceDate, targetDate, jobId);
  };

  const handleMoveToDispatch = (dateString: string) => {
