...
export const Card: React.FC<{ children: React.ReactNode, className?: string, onClick?: () => void }> = ({ children, className = '', onClick }) => (
  <div onClick={onClick} className={`tw-bg-white tw-rounded-xl tw-shadow-sm tw-border tw-border-slate-200 tw-p-4 ${className}`}>
    {children}
  </div>
);
...
export const Modal: React.FC<{ isOpen: boolean; onClose: () => void; title: string; children: React.ReactNode }> = ({ isOpen, onClose, title, children }) => {
  ...
  return (
    <div className="tw-fixed tw-inset-0 tw-z-50 tw-flex tw-items-end sm:tw-items-center tw-justify-center tw-bg-black/60 tw-backdrop-blur-sm tw-animate-fade-in sm:tw-p-4">
...
export const Toast: React.FC<{ message: string | null; type?: 'success' | 'error' | 'info'; onClose: () => void }> = ({ message, type = 'success', onClose }) => {
  ...
  return (
    <div className="tw-fixed tw-top-4 tw-left-4 tw-right-4 tw-z-[60] tw-animate-slide-up">
