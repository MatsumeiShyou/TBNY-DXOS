    ...
    return (
      <div className="tw-flex tw-flex-col tw-h-full">
        <div className="tw-flex-1 tw-p-4 tw-space-y-6 tw-overflow-y-auto">
          ...
          <Card className="tw-bg-blue-50 tw-border-blue-200" agentId="summary-card">
      ...
      <div className="tw-flex tw-flex-col tw-h-full tw-bg-slate-50">
        <div className="tw-flex-1 tw-p-4 tw-overflow-y-auto tw-pb-80">
          ...
          <div className="tw-space-y-4">
            {items.map(item => (
              <Card 
                key={item.id} 
                className={`${item.isCollected ? 'tw-border-primary tw-ring-1 tw-ring-blue-100 tw-shadow-md' : ''} tw-transition-all`}
        ...
        <Modal title="品目追加" isOpen={isAddItemModalOpen} onClose={() => setAddItemModalOpen(false)} agentId="add-item-modal">
          <div className="tw-space-y-6">
            ...
            <Button onClick={addNewItem} className="tw-mt-4" agentId="add-item-modal:execute-button">追加する</Button>
          </div>
        </Modal>
      </div>
    );
