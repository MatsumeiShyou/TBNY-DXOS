...
import type { MasterSchema, MasterColumn, MasterField } from '../config/masterSchema';
...
interface ExtendedMasterField extends Omit<MasterField, 'lookup'> {
    placeholder?: string;
    ...
    const handleEdit = async (item: Record<string, unknown>) => {
        ...
        if (schema.viewName !== schema.rpcTableName) {
            try {
                ...
                const detail = results?.[0] as Record<string, unknown> | undefined;
        ...
        <div className="tw-h-full tw-flex tw-flex-col tw-bg-slate-50 tw-dark:bg-slate-950">
            ...
            >
                {isDeepFetching ? (
                    ...
                    <MasterForm
                        key={editingItem ? String(editingItem[schema.primaryKey]) : 'new'}
                        schema={schema}
                        initialData={editingItem}
                        onSave={handleSave}
                        onDelete={editingItem ? () => handleDelete(String(editingItem[schema.primaryKey])) : undefined}
...
function renderCell(item: Record<string, unknown>, col: MasterColumn) {
                ...
                <div className="tw-flex tw-items-center tw-gap-2">
                    ...
                    <div className="tw-flex tw-gap-0.5">
                        {!!item['site_contact_phone'] && (
                            <div className="tw-w-5 tw-h-5 tw-flex tw-items-center tw-justify-center tw-rounded-full tw-bg-blue-50 tw-dark:bg-blue-900/30 tw-text-blue-600 tw-dark:text-blue-400" title={String(item['site_contact_phone'])}>
                        ...
                        {!!item['internal_note'] && (
                            <div className="tw-w-5 tw-h-5 tw-flex tw-items-center tw-justify-center tw-rounded-full tw-bg-amber-50 tw-dark:bg-amber-900/30 tw-text-amber-600 tw-dark:text-amber-400" title={String(item['internal_note'])}>
    ...
    return (
        <span className="tw-text-slate-600 tw-dark:text-slate-400">
            {col.optionLabels?.[String(value)] || (value as React.ReactNode) || '-'}
        </span>
    );
}
        ...
        <form onSubmit={handleSubmit} className="tw-flex tw-flex-col tw-h-full">
            <div className="tw-grid tw-grid-cols-1 tw-md:grid-cols-2 tw-gap-x-6 tw-gap-y-4">
                {schema.fields.map(field => {
                    const fExt = field as ExtendedMasterField;
                    return (
                        <div key={field.name} className={`${field.className || ''} flex flex-col`}>
                                ...
                                <div className="tw-space-y-2">
                                    ...
                                    <div className="tw-flex tw-flex-wrap tw-gap-1.5 tw-max-h-32 tw-overflow-y-auto tw-p-1">
                                        {allItems.map(item => {
                                            ...
                                            const isSelected = String(formData[field.name] || '').split(',').map(s => s.trim()).includes(nameStr);
                                            return (
                                                <button
                                                    key={String(item['id'])}
                                                    type="button"
                                                    onClick={() => toggleTag(field.name, nameStr)}
                            ...
                            ) : field.type === 'select' && fExt.lookup ? (
                                <LookupSelect
                                    field={fExt}
                                    value={String(formData[field.name] || '')}
                                    onChange={(val: string) => setFormData({ ...formData, [field.name]: val })}
                                />
                            ) : field.type === 'select' ? (
                                <select
                                ...
                                <div className="tw-space-y-4 tw-p-2 tw-bg-slate-50/50 tw-dark:bg-slate-900/50 tw-rounded-xl tw-border tw-border-slate-100 tw-dark:border-slate-800">
                                    ...
                                    <div className="tw-pt-3 tw-border-t tw-border-slate-200 tw-dark:border-slate-700">
                                        ...
                                        <div className="tw-flex tw-items-center tw-gap-2 tw-mb-2">
                                            ...
                                            >
                                                {[1, 2, 3, 4, 5].map(w => <option key={w} value={String(w)}>第{w}週</option>)}
                                            </select>
                                            <select
                                                aria-label="曜日の選択"
                                                className="tw-px-2 tw-py-1.5 tw-text-xs tw-rounded-lg tw-bg-white tw-dark:bg-slate-800 tw-border-2 tw-border-slate-200 tw-dark:border-slate-700 tw-focus:ring-1 tw-focus:border-indigo-500 tw-focus:ring-indigo-500 tw-text-slate-700 tw-dark:text-white"
            ...
            {schema.rpcTableName === 'master_collection_points' && initialData && (
                <PointAccessSection pointId={String(initialData.id)} />
            )}
...
function LookupSelect({ field, value, onChange }: {
    ...
    const valKey = field.lookup.valueKey;
    const lblKey = field.lookup.labelKey;
    ...
    return (
        ...
        >
            ...
            {options.map((opt) => (
                <option key={String(opt[valKey])} value={String(opt[valKey])}>
                    {String(opt[lblKey])}
                </option>
            ))}
        </select>
...
function PointAccessSection({ pointId }: { pointId: string }) {
    ...
    const [drivers, setDrivers] = useState<SimpleStaff[]>([]);
    ...
    useEffect(() => {
        ...
        supabase.from('point_access_permissions')
            ...
            .then(({ data }) => setPermissions((data || []) as unknown as PointAccessPermission[]));
        // スタッフ（旧ドライバー）一覧
        ...
        supabase.from('vehicles').select('id, number, callsign').then(({ data }) =>
            setVehicles((data || []) as unknown as SimpleVehicle[])
        );
    }, [isOpen, pointId]);
    ...
    const handleAdd = async () => {
        ...
        const { data } = await supabase.from('point_access_permissions')
            .select('*, profile:staffs(id, name), vehicle:vehicles(id, callsign, number)')
            .eq('point_id', pointId).eq('is_active', true);
        setPermissions((data || []) as unknown as PointAccessPermission[]);
        setSaving(false);
    };
