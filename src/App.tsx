...
import { generateDailySchedule, getDummyDate } from './utils/calendarUtils';
...
export default function App() {
  ...
  const [dragOverErrorReason, setDragOverErrorReason] = useState<string | null>(null);
  const [draggingPendingJob, setDraggingPendingJob] = useState<any>(null);
  const [pendingJobOffsetY, setPendingJobOffsetY] = useState<number>(0);
  ...
  const startPreview = (templateData) => {
    ...
    const normalizeJobs = (jobs: any[]) => jobs.map(j => ({ ...j, startTime: j.startTime ? j.startTime.replace(/^0/, '') : j.startTime }));

    let finalJobs = normalizeJobs(templateData.state.jobs || []);
    let finalPendingJobs = normalizeJobs(templateData.state.pendingJobs || []);

    // テンプレートに曜日属性がある場合、未配車リストを動的に補完する
    if (templateData.targetWeek && templateData.targetDay) {
      const dummyDate = getDummyDate(templateData.targetWeek, templateData.targetDay);
      // その日の本来あるべき全案件を生成
      const expectedJobs = generateDailySchedule(dummyDate, masterCustomers, [], []);
      
      // 既に盤面や未配車リストにある顧客IDを抽出
      const existingCustomerIds = new Set([
        ...finalJobs.map(j => j.originalCustomerId),
        ...finalPendingJobs.map(j => j.originalCustomerId)
      ]);

      // 足りない案件を抽出し、pendingJobsに追加
      const missingJobs = expectedJobs.filter(j => !existingCustomerIds.has(j.originalCustomerId));
      
      if (missingJobs.length > 0) {
        finalPendingJobs = [...finalPendingJobs, ...missingJobs];
      }
    }

    // テンプレートデータを展開
    setJobs(finalJobs);
    setPendingJobs(finalPendingJobs);
    ...
    setPreviewingTemplate({ 
      ...
      targetDay: templateData.targetDay 
    });
  ...
  const savePreviewToTemplate = async () => {
    ...
    const updatedTemplate = {
      ...
      targetWeek: previewingTemplate.targetWeek,
      targetDay: previewingTemplate.targetDay,
      state: {
        jobs,
        pendingJobs,
        splits,
  ...
  const [viewMode, setViewMode] = useState<'dispatch' | 'calendar'>('dispatch');
  // --- マスタCRUDハンドラ ---
  ...
  const [relativeOffsetY, setRelativeOffsetY] = useState(0);
  const [dragCurrent, setDragCurrent] = useState({ x: 0, y: 0 });
  const [dragMousePos, setDragMousePos] = useState({ x: 0, y: 0 });
  
  const [resizingState, setResizingState] = useState(null);

  const driverColRefs = useRef<Record<string, HTMLDivElement | null>>({});

  // ----------------------------------------
  // コース管琁EジチE
  // ----------------------------------------
  const handleAddCourse = ({ course, name, currentVehicle, color }: any) => {
    recordHistory();
  ...
  const openSplitEdit = (e, driverId, time) => {
    ...
    setEditModal({
      ...
      targetId: driverId,
      time: time,
      initialDriverName: split ? split.newDriverName : (driver?.name || ''),
      initialVehicle: split ? split.newVehicleName : (driver?.currentVehicle || '')
    });
  };
    ...
    if (editModal.type === 'header') {
      ...
      setSplits(prev => {
        ...
        if (idx >= 0) {
          ...
          newSplits[idx] = { ...newSplits[idx], newDriverName: newName, newVehicleName: newVehicle };
          return newSplits;
        } else {
          return [...prev, { id: `split_${editModal.targetId}_${Date.now()}`, driverId: editModal.targetId, time: editModal.time, newDriverName: newName, newVehicleName: newVehicle }];
        }
      });
    }
    setEditModal(null);
  ...
  const handleContextMenu = (e, driverId, time) => {
    ...
    setSplits(prev => {
        ...
        if (existingIndex >= 0) {
            ...
            const driver = drivers.find(d => d.id === driverId);
            return [...prev, { id: `split_${driverId}_${Date.now()}`, driverId, time, newDriverName: driver?.name || '未定', newVehicleName: driver?.currentVehicle || '車両' }];
        }
    });
  };

  const calculateDropTarget = (clientX, clientY, targetJobOrId, offsetY) => {
    const targetJob = typeof targetJobOrId === 'string' ? jobs.find(j => j.id === targetJobOrId) : targetJobOrId;
    ...
    if (!gridTop && driverColRefs.current[newDriverId]) {
      gridTop = driverColRefs.current[newDriverId].getBoundingClientRect().top;
    }
    
    const cardTopY = clientY - offsetY;
    const diffY = cardTopY - gridTop;
    const moveYBlocks = Math.round(diffY / CELL_HEIGHT_PX);
    
    let newStartMin = timeToMinutes('06:00') + (moveYBlocks * 15);
    ...
    let overlapErrorReason = '';

    const driverSplits = splits.filter(s => s.driverId === newDriverId);
    const splitAtStart = driverSplits.find(s => s.time === newStartTime);
    if (splitAtStart) {
      isOverlapError = true;
      overlapErrorReason = '乗り換え時間と重複しています';
    }

    const otherJobs = jobs.filter(j => j.driverId === newDriverId && j.id !== targetJob.id);
    ...
    if (isStartOverlapping && !isOverlapError) {
      isOverlapError = true;
      overlapErrorReason = '他の回収案件と重複しています';
      ...
      if (nearestObstacleStart !== 99999) {
          ...
          if (availableDuration < 15) {
            isOverlapError = true;
            overlapErrorReason = '空き時間が15分未満です';
            newDuration = 15; 
          } else {
    ...
    for (const split of sortedSplits) {
        if (timeToMinutes(split.time) <= newStartMin) currentVeh = split.newVehicleName;
        else break; 
    }
    ...
    console.log('[DEBUG-DnD]', { 
      clientX, clientY, 
      offsetY, gridTop, moveYBlocks, newStartMin, newStartTime, 
      isOverlapError, overlapErrorReason, newDuration,
      targetJobId: targetJob?.id, newDriverId,
      otherJobsCount: otherJobs.length,
      isStartOverlapping
    });

    return { driverId: newDriverId, startTime: newStartTime, duration: newDuration, isVehicleError, isOverlapError, overlapErrorReason };
  };

  const calculateSplitDropTarget = (clientX, clientY, splitId, offsetY) => {
  ...
  useEffect(() => {
    const handleMouseMove = (e) => {
      ...
      if (draggingJobId) {
        ...
        const currentY = e.clientY - dragOffset.y; // dragCurrent更新用
        setDragCurrent({ x: currentX, y: currentY });
        setDropPreview(calculateDropTarget(e.clientX, e.clientY, draggingJobId, relativeOffsetY));
      }
      ...
      if (draggingSplitId) {
        ...
        setDragCurrent({ x: currentX, y: currentY });
        setDropSplitPreview(calculateSplitDropTarget(e.clientX, e.clientY, draggingSplitId, relativeOffsetY));
      }
    };

    const handleMouseUp = (e) => {
      console.log('[DEBUG-MOUSEUP] draggingJobId:', draggingJobId, 'relativeOffsetY:', relativeOffsetY, 'dragOffset.y:', dragOffset.y, 'e.clientY:', e.clientY);
      if (resizingState) {
        recordHistory(); 
      ...
      if (draggingJobId) {
        ...
        if (targetElement && targetElement.closest('#pending-jobs-dock')) {
          const originalJob = jobs.find(j => j.id === draggingJobId);
          if (originalJob) {
            recordHistory();
            setPendingJobs(prev => [...prev, originalJob]);
            setJobs(prev => prev.filter(j => j.id !== draggingJobId));
            setSelectedJobId(null);
            showToast('案件を未配車リストに戻しました', 'info');
          }
        ...
        const preview = calculateDropTarget(e.clientX, e.clientY, draggingJobId, relativeOffsetY);
        const originalJob = jobs.find(j => j.id === draggingJobId);
        if (preview && !preview.isOverlapError && originalJob) {
          recordHistory();
          if (preview.duration < originalJob.duration) {
            showToast('空き時間に合わせて所要時間を自動短縮しました', 'warning');
          }
      ...
      if (draggingSplitId) {
        const preview = calculateSplitDropTarget(e.clientX, e.clientY, draggingSplitId, relativeOffsetY);
  ...
  }, [resizingState, draggingJobId, draggingSplitId, dragOffset, relativeOffsetY, jobs, splits, dragButton, dragMousePos, recordHistory]);
  ...
  const handleAddJobDirect = (jobTemplate, targetDriverId, targetTime) => {
    ...
    for (const s of driverSplits) {
        if (timeToMinutes(s.time) <= newStartMin) currentVeh = s.newVehicleName;
        else break; 
    }
  ...
  return (
    <div className={`flex flex-col h-screen ${previewingTemplate ? 'bg-purple-50 border-4 border-purple-500' : 'bg-white'} text-sm font-sans text-gray-800 select-none transition-colors duration-300`}>
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="flex">
            ...
            <div 
              ...
              onDragOver={(e) => {
                e.preventDefault();
                if (!draggingPendingJob) return;
                const preview = calculateDropTarget(e.clientX, e.clientY, draggingPendingJob, pendingJobOffsetY);
                if (preview) {
                  setDragOverErrorReason(preview.overlapErrorReason);
                  setDropPreview(preview);
                }
              }}
              onDragLeave={() => {
                setDragOverErrorReason(null);
                setDropPreview(null);
              }}
              onDrop={(e) => {
                e.preventDefault();
                ...
                const preview = calculateDropTarget(e.clientX, e.clientY, draggingPendingJob, pendingJobOffsetY);
                if (preview && !preview.isOverlapError) {
                  handleDropPendingJob(e, preview.driverId, preview.startTime); // <- ここで正しい位置を渡す
                }
                setDraggingPendingJob(null);
              }}
            >
              {drivers.map((driver) => {
                let activeDraggingJob = draggingPendingJob;
                if (!activeDraggingJob && draggingJobId) {
                  activeDraggingJob = jobs.find(j => j.id === draggingJobId);
                }
                const dragReqVeh = activeDraggingJob?.requiredVehicle;
                // ※ この列の currentVehicle は時間帯によって(Split)変わる可能性があるが、簡略化のため基本の currentVehicle で判定
                const isMismatch = dragReqVeh && driver.currentVehicle && dragReqVeh !== driver.currentVehicle;

                return (
                  <div 
                    key={driver.id} 
                    className={`w-[180px] border-r border-gray-300 relative transition-opacity duration-200 ${isMismatch ? 'opacity-40 bg-gray-100 grayscale' : ''}`} 
                    ref={el => { driverColRefs.current[driver.id] = el; }}
                  >
                    {TIME_SLOTS.map((time) => {
                    ...
                    return (
                      <div key={time} className={`h-8 ${borderClass} relative`} onContextMenu={(e) => handleContextMenu(e, driver.id, time)}>
                        ...
                        <SplitLine 
                          ...
                          setRelativeOffsetY={setRelativeOffsetY}
                        ...
                        {isPreviewStart && (
                          <div className={`absolute left-0 right-0 z-30 border-2 rounded pointer-events-none flex flex-col p-1 shadow-sm transition-all duration-75 ${dropPreview.isOverlapError ? 'bg-red-200/90 border-red-600 z-50' : dropPreview.isVehicleError ? 'bg-red-50/90 border-red-400' : 'bg-emerald-50/90 border-emerald-400'}`} style={{ top: 0, height: `${(dropPreview.duration / 15) * QUARTER_HEIGHT_REM}rem` }}>
                             <div className="flex justify-between items-start">
                               <div className={`text-[10px] font-bold px-1.5 py-0.5 rounded flex items-center gap-1 ${dropPreview.isOverlapError ? 'bg-red-600 text-white shadow' : dropPreview.isVehicleError ? 'bg-red-100 text-red-700' : 'bg-white/80 text-emerald-800'}`}>
                                 {dropPreview.isOverlapError && <Ban size={10} />}
                                 {dropPreview.isOverlapError ? `移動不可${dropPreview.overlapErrorReason ? `: ${dropPreview.overlapErrorReason}` : ''}` : `${dropPreview.startTime} (${dropPreview.duration}分)`}
                               </div>
      ...
      <PendingJobsDock 
        ...
        onDragStartJob={(job, offsetY) => {
          setDraggingPendingJob(job);
          if (offsetY !== undefined) setPendingJobOffsetY(offsetY);
        }}
        onDragEndJob={() => {
          setDraggingPendingJob(null);
          setDragOverErrorReason(null);
