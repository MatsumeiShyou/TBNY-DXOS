...
2. **未配車リストドック (Pending Jobs)**
   - 画面右側に常時表示される未配車リストのドックから、ドラッグ＆ドロップで案件を直接配車盤へ割り当てることができます。
   - 必要な車両指定（例: `2025PK` 限定）がある場合、アイコンで警告が表示されます。
   - 不要なジョブは再度未配車リストへ戻すことも可能です。
3. **スケジュール調整カレンダー自動展開機能**
   - ヘッダーの切り替えボタンから「カレンダー」ビューへ移動できます。
   - 顧客マスタに登録された回収頻度（第1・第4・毎週等）に基づいて、指定月のスケジュールを自動的に生成・展開します。
   - **手動調整**: ドライバーが回収先からヒアリングした休業予定などを反映するため、カレンダー上でドラッグ＆ドロップによる日程変更（ズラし）や、削除によるスキップが可能です。
   - 調整が確定した日程の案件は、「転送」ボタンにより未配車リストへ流し込み、当日の配車パズルに利用できます。
4. **全体休業日設定**
